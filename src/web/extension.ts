...
export function activate(context: vscode.ExtensionContext) {
			...
			async (message) => {
				switch (message.command) {
					...
					case 'deleteTable':
						...
						break;
					case 'saveDatabase':
						await handleSaveDatabase(panel, message.useIfNotExists || false, message.saveFormat || 'mssql');
						break;
	...
	function generateDBMLContent(): string {
		if (!currentDatabase) return '';

		let dbml = `// Database: ${currentDatabase.name}\n`;
		dbml += `// Generated by SQLGem at ${new Date().toISOString()}\n\n`;
	...
	async function handleSaveDatabase(panel: vscode.WebviewPanel, useIfNotExists: boolean = false, saveFormat: string = 'mssql'): Promise<void> {
		if (!currentDatabase) {
		...
		try {
			await writeDatabaseSQL(useIfNotExists, saveFormat);
			const fileName = saveFormat === 'dbml' ? 'database.dbml' : 'database.sql';
			vscode.window.showInformationMessage(`Database "${currentDatabase.name}" saved to ${fileName}`);
		} catch (error) {
	...
	async function writeDatabaseSQL(useIfNotExists: boolean = false, saveFormat: string = 'mssql'): Promise<void> {
		...
		if (saveFormat === 'dbml') {
			const dbmlFileUri = vscode.Uri.joinPath(currentDatabaseFolderUri, 'database.dbml');
			const dbmlContent = generateDBMLContent();
			const encoder = new TextEncoder();
			await vscode.workspace.fs.writeFile(dbmlFileUri, encoder.encode(dbmlContent));
		} else {
